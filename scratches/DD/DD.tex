...
This algorithm calculates the best proposal for a user that wants to use
the MoneySavingOption. It binds a weight to each station that considers
not only the proximity to the destination but also the number of
available plugs, that is to keep a good distribution of the cars among
the city. If there's no reasonable choice the algorithm return an error
coordinate.
...
In the end we have the auxiliary part of the previous algorithms. First
we have a method to get a list of the first \emph{n} recharging stations
closer to a position. Then a trivial method to compute the weight of a
station given the point of interest, and in the end an auxiliary method
used to update the list of stations closer to a position keeping them in
